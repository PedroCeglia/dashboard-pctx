...
export default function FilterProvider({ children, tableList }) {
  ...
  function filterListByType(filterType, filterText, mainList) {
    ...
    return mainList.filter((item) => {
      if (item[filterType] != null)
        if (item[filterType].toUpperCase().indexOf(filterText.toUpperCase()) != -1) return item;
    });
  }
  ...
  return (
    <FilterContext.Provider
      value={{ uiTableList, addItemFilter, removeItemFilter }}
    >
      {children}
    </FilterContext.Provider>
  );
